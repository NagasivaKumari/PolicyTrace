...
contracts_project_root = root_path.parent
backend_env_path = contracts_project_root.parent / "blockd-backend" / ".env"

# Single source of truth: load backend env first.
if backend_env_path.exists():
    logger.info(f"Loading env from {backend_env_path}")
    load_dotenv(dotenv_path=backend_env_path, override=False)
else:
    logger.info("Backend .env not found; falling back to local .env if present")
    load_dotenv()
...
def _get_output_path(output_dir: Path, deployment_extension: str) -> Path:
    return output_dir / Path("{contract_name}" + ("_client" if deployment_extension == "py" else "Client") + f".{deployment_extension}")
...
def main(action: str, contract_name: str | None = None) -> None:
    ...
    match action:
        ...
        case "deploy":
            did_deploy_any = False
            for contract in filtered_contracts:
                # Ensure app specs are up to date before deployment.
                build(artifact_path / contract.name, contract.path)
                if contract.deploy:
                    logger.info(f"Deploying app {contract.name}")
                    contract.deploy()
                    did_deploy_any = True
            if not did_deploy_any:
                logger.warning("No deploy_config.py found. Add deploy_config.py in each contract folder to enable app deployment.")
        case "all":
            for contract in filtered_contracts:
                logger.info(f"Building app at {contract.path}")
                build(artifact_path / contract.name, contract.path)
